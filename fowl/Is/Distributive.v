...
Class IsDistr (A B : Type) (R : B -> B -> Prop) (f : A -> B)
  (Hk : HasBinOp A) (Hm : HasBinOp B) : Prop :=
  distr (x y : A) : R (f (x + y)) (f x + f y).

Class IsAntidistr (A B : Type) (R : B -> B -> Prop) (f : A -> B)
  (Hk : HasBinOp A) (Hm : HasBinOp B) : Prop :=
  antidistr (x y : A) : R (f (x + y)) (f y + f x).
...
Class IsDistrL (A : Type) (R : A -> A -> Prop)
  (Hk : HasMul A) (Hm : HasAdd A) : Prop :=
  distr_l (x y z : A) : R (x * (y + z)) (x * y + x * z).

(** This has the same shape as [mul_add_distr_r]. *)

Class IsDistrR (A : Type) (R : A -> A -> Prop)
  (Hk : HasMul A) (Hm : HasAdd A) : Prop :=
  distr_r (x y z : A) : R ((x + y) * z) (x * z + y * z).

Class IsDistrLR (A : Type) (R : A -> A -> Prop)
  (Hk : HasMul A) (Hm : HasAdd A) : Prop := {
  is_distr_l :> IsDistrL R _*_ _+_;
  is_distr_r :> IsDistrR R _*_ _+_;
}.
