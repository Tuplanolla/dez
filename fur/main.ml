...
let main () =
  ...
  begin fun oc ->
    ...
    | _ :: [] -> Server_proxy.start ()
    | _ :: addr :: [] -> Server_proxy.start ~addr:addr ()
    | _ :: addr :: port :: [] ->
        Server_proxy.start ~addr:addr ~port:(int_of_string port) ()
    | _ -> raise (Invalid_argument "fur")
  end
