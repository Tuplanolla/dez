...
Module Export ReflexiveClass.

Delimit Scope setoid_scope with setoid.

Open Scope setoid_scope.

Class Rel (A : Type) : Type := rel : A -> A -> Prop.
...
Instance EqvRel {A : Type} {eqv : Eqv A} : Rel A := eqv.
...
Class Setoid (A : Type) {eqv : Eqv A} : Prop := {
  reflexive :> ReflexiveClass.Reflexive A;
  symmetric :> SymmetricClass.Symmetric A;
  transitive :> TransitiveClass.Transitive A;
}.
(* end snippet setoid *)

Add Parametric Relation {A : Type} `{setoid : Setoid A} : A eqv
  reflexivity proved by ReflexiveClass.reflexive
  symmetry proved by SymmetricClass.symmetric
  transitivity proved by TransitiveClass.transitive
  as eqv_relation.
