...
Class IsBimodule {LS RS A : Type} {LS_has_eqv : HasEqv LS}
  ...
  bimodule_add_zero_neg_mul_one_opr_idn_inv_lsmul_is_left_module :>
    IsLeftModule add zero neg mul one opr idn inv lsmul;
  bimodule_add_zero_neg_mul_one_opr_idn_inv_rsmul_is_right_module :>
    IsRightModule add zero neg mul one opr idn inv rsmul;
  bimodule_biassociative : forall (a : LS) (b : RS) (x : A),
    (a <* x) *> b == a <* (x *> b);
}.
